...
sys.path.append("/data/yhy/ocr/ocrmap/src/parking_slot_detection/scripts/gcn-parking-slot")
...
class gcn_detector_server():
    ...
    def handle_request(self, req):
        ...
        cropped_image = image[center_y - crop_size1 // 2 : center_y + crop_size1 // 2,
                            center_x - crop_size1 // 2 : center_x + crop_size1 // 2]
        # image1 = image[0 : center_y * 2,
        #                      center_x - crop_size // 2 : center_x + crop_size // 2]
        # cropped_image1 = image[center_y - crop_size1 // 2 : center_y + crop_size1 // 2,
        #                     center_x - crop_size // 2 : center_x + crop_size // 2]

        # # 缩放图像到 512x512
        image2 = cv2.resize(cropped_image, (512, 512))
      ...
      # 遍历边界框
        ...
        if image2.shape[0] != self.input_image_size[0] or image2.shape[1] != self.input_image_size[1] or image2.shape[2] != self.input_image_size[2]:
            print("image size error!!! reciving image size is: ", image2.shape)
            return point0_x, point0_y, point1_x, point1_y, types
        image2 = image2/255.0
        data_dict = {}
        data_dict['image'] = torch.from_numpy(image2).float().permute(2, 0, 1).unsqueeze(0).cuda()
        with torch.no_grad():
        ...
        # 计算其余两个点
          ...
          # text_with_box.stamp=gps_1
                ...
                pts = np.array([(p0_x,p0_y), (p1_x,p1_y), (p2_x,p2_y), (p3_x,p3_y)], np.int32)
                with torch.no_grad():
                    label_vacant = vps_classify.vps_classify(image3, pts)
                    # labelvacant1=int(label_vacant)
                    label.append(label_vacant)
            ...
            return point0_x, point0_y, point1_x, point1_y,point2_x, point2_y,point3_x, point3_y, types,label,ocrpointx1,ocrpointx2,ocrpointy1,ocrpointy2,texts,confidence
...
if __name__ == '__main__':
    ...
    vps_classify = vpsClassify("/data/yhy/ocr/ocrmap/src/parking_slot_detection/scripts/gcn-parking-slot/model/Customized.pth", device)
