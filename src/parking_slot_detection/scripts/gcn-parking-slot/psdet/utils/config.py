...
def get_args():
    ...
    argparser.add_argument( '-c', '--cfg', metavar='C', required=False, default="../config/ps_gat.yaml", help='The Configuration file')
    argparser.add_argument( '-s', '--seed', default=100, type=int, help='The random seed')
    argparser.add_argument( '-m', '--ckpt', type=str, default="../model/checkpoint_epoch_200.pth", help='The model path')
    ...
    args.cfg = "/data/yhy/ocr/ocrmap/src/parking_slot_detection/scripts/gcn-parking-slot/config/ps_gat.yaml"
    args.ckpt = "/data/yhy/ocr/ocrmap/src/parking_slot_detection/scripts/gcn-parking-slot/model/checkpoint_epoch_200.pth"
    return args
...
def get_config():
    ...
    # print("abs path: ", os.path.abspath(config_file))
