...
ros::Publisher license_marker_pub;
ros::Publisher slots_boxes_pub;
...
std::vector<showtrajectory::Slots::ConstPtr> slots_msgs;
std::vector<showtrajectory::OCRResult::ConstPtr> number_msgs;
...
/*void drawmapOnCV(const nav_msgs::Path& path, const std::vector<showtrajectory::OCRResult::ConstPtr>& slots_msgs, const std::vector<showtrajectory::OCRResult::ConstPtr>& number_msgs) {
    canvas.setTo(cv::Scalar(255, 255, 255)); // 清空画布

    // 绘制车辆轨迹
    for (const auto& pose : path.poses) {
        cv::Point point(pose.pose.position.x + canvas.cols / 2, pose.pose.position.y + canvas.rows / 2);
        cv::circle(canvas, point, 2, cv::Scalar(0, 0, 255), cv::FILLED); // 蓝色点
    }

    // 绘制车位框
    for (const auto& ocr_msg : slots_msgs) {
        const std::vector<geometry_msgs::Point32>& vertices = ocr_msg->polygon.points;
        for (size_t i = 0; i < vertices.size(); i++) {
            cv::Point p1(vertices[i].y * cos(yaw) - vertices[i].x * sin(yaw) +pose.pose.position.x+ canvas.cols / 2,
                         vertices[i].y * sin(yaw) + vertices[i].x * cos(yaw) + pose.pose.position.y+canvas.rows / 2);
            cv::Point p2(vertices[(i + 1) % vertices.size()].y * cos(yaw) - vertices[(i + 1) % vertices.size()].x * sin(yaw) + pose.pose.position.x+canvas.cols / 2,
                         vertices[(i + 1) % vertices.size()].y * sin(yaw) + vertices[(i + 1) % vertices.size()].x * cos(yaw) + pose.pose.position.y+canvas.rows / 2);
            cv::line(canvas, p1, p2, cv::Scalar(0, 255, 0), 2); // 绿色框
        }
    }

    // 绘制车位号框和车位号
    for (const auto& ocr_msg : number_msgs) {
        const std::vector<geometry_msgs::Point32>& vertices = ocr_msg->polygon.points;
        for (size_t i = 0; i < vertices.size(); i++) {
            cv::Point p1(vertices[i].y * cos(yaw) - vertices[i].x * sin(yaw) + pose.pose.position.x+canvas.cols / 2,
                         vertices[i].y * sin(yaw) + vertices[i].x * cos(yaw) +pose.pose.position.y+ canvas.rows / 2);
            cv::Point p2(vertices[(i + 1) % vertices.size()].y * cos(yaw) - vertices[(i + 1) % vertices.size()].x * sin(yaw) +pose.pose.position.x+ canvas.cols / 2,
                         vertices[(i + 1) % vertices.size()].y * sin(yaw) + vertices[(i + 1) % vertices.size()].x * cos(yaw) +pose.pose.position.y+ canvas.rows / 2);
            cv::line(canvas, p1, p2, cv::Scalar(255, 0, 0), 2); // 红色框
        }
        ...
        center.x = center.x / vertices.size() + canvas.cols / 2;
        center.y = center.y / vertices.size() + canvas.rows / 2;

        cv::putText(canvas, ocr_msg->text, center, cv::FONT_HERSHEY_SIMPLEX, 1.0, cv::Scalar(0, 0, 255), 2); // 红色文字
    }
